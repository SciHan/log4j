 ...
 * <p>
 * he SocketAppender has the following properties:
 *
 * <ul>
 *
    ...
    public void append(LoggingEvent event) {
        ...
        if (oos != null) {
            try {
                ...
                //一直不明白这种为什么不接受返回值，却又能把值设进去，现在看，这个设计也是非常好的，具体可以debug
                // SocketAppenderTest中的test 看似没有接受返回值，实际上都把返回值设置进去了，因为调用的是event，写入的
                // 同样是event
                event.getNDC();
                event.getThreadName();
                event.getMDCCopy();
