...
public class DailyRollingFileAppender extends FileAppender {
    ...
    void rollOver() throws IOException {
        ...
        // 检查是否还在同一个时间间隔内，如果还在，那么就先return，比如你14：00 1秒初始化log4j，设定的间隔是一分子，那么
        // 系统在14：00 30秒的时候迎来了第一条日志的打印，第一条日志的打印是必会进入到这个循环的，因为还没有修正下次检查的
        // 时间，所以此时其实还在间隔内，那么这里就会进行return
        if (scheduledFilename.equals(datedFilename)) {
            return;
        }

        // 进入后续逻辑代表已经够间隔了 疑问：如果多个appender并发执行的时候会不会出现线程安全问题
        //  感觉会出现线程不安全问题 首先当多个FileAppender对同一个文件进行操作的话，很可能会出现线程安全问题
        // 在test中测试了一下 的确可能会出现多个线程 同时操作文件的情况 会发生error的情况 可以说是一个设计不够严谨的地方把，
        // 不过还是那句，这种情况出现的概率 真的是非常少见
        // close current file, and rename it to datedFilename
        this.closeFile();

        //新建file 这个file准备接受过去间隔时间的日志
        File target = new File(scheduledFilename);
        //如果存在，那么先删除（疑问 : 存在的话为什么直接先删除？
        // 答：这里是自己对文件操作不熟悉了，删除了已有的，只要保留了新建的文件对象，下面的renameTo方法依然可以转移的）
        if (target.exists()) {
            target.delete();
        }
        ...
        //这个为过去时间间隔中拥有的日志文件
        File file = new File(fileName);
        //下面的renameTo方法，即将过去时间间隔的文件移动到新建的file中
        //renameTo的方法其实有点歧义，这里的意思是将文件从一个路径移动另一个路径下，并且移动的文件进行重命名【文件移动重命名】
        //可以参考 https://blog.csdn.net/u010648555/article/details/78356040 这篇博客
    ...
    protected void subAppend(LoggingEvent event) {
        ...
        //第一次是一定会进行if循环内的 然后通过rc.getNextCheckMillis(now);方法就会将nextcheck的时间修正，待下次
        // 满足检查时间的时候才进入循环，比如设定的是每分钟分离一次日志操作，在第一次检查完成修正时间过后，下一次的检查
        // 时间就是第二次
        if (n >= nextCheck) {
            now.setTime(n);
