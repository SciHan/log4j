...
public class WriterAppender extends AppenderSkeleton {
    ...
    public void append(LoggingEvent event) {
        ...
        //这里是验证一些参数 如appender是否已经close close则直接返回 是否有quiteWriter，
        // 可以理解为OutputStream是否为空 还有即为layout是否为空 layout即为格式化后的日志
        // 总的来说这个方法是一些参数的检查
    ...
    protected void subAppend(LoggingEvent event) {
        // layout.format 即格式化日志事件 将event转化为输出的日志格式
        // 同时 qw还是决定输出的方式 appender中的实际逻辑，即是输出到console还是输出到文件 实际上就是通过设置
        // 这个qw来实现的 比如需要设置consoleAppender 那么在consoleAppender实例化的时候 就会去设置qw
        // 所以在源码进行debug的时候哦 看到consoleAppender中的逻辑实际上是在WriterAppender类中的时候 可能
        // 会有些迷糊 但实际上WriterAppender中的qw属性 是由consoleAppender进行设置的 这应该是一种设计模式的实现
        // 具体的真正逻辑 待理清 todo
        this.qw.write(this.layout.format(event));

        //异常处理 根据layout的种类判断是否处理
